...
**IMPORTANT:** For feature-specific error handling patterns (mandatory), see [errors.md](./errors.md). All API errors should use custom error classes (e.g., `UserError`, `AuthError`) instead of generic `IgniterResponseError`.
...
notifications: igniter.query({
  ...
  handler: async ({ context, realtime }) => {
    ...
    const subscription = igniter.store.subscribe(
...
**CRITICAL:** Always use feature-specific custom error classes (see [errors.md](./errors.md)) instead of generic errors. This ensures type-safe error handling with autocomplete support for error codes.

### 6.1. Error Structure

All API errors follow this structure (auto-generated from feature error classes):

```typescript
interface ApiError {
  code: string; // e.g., "USER_NOT_FOUND", "INVALID_CREDENTIALS"
  message: string; // Human-readable error message
  statusCode: number; // HTTP status code (404, 401, etc.)
  details?: unknown; // Additional error context
  metadata?: Record<string, unknown>; // Request metadata
  causer?: string; // Function that threw the error
}
```
...
**IMPORTANT:** Always use type-safe error codes from your feature error classes (see [errors.md](./errors.md)).
...
export function UserProfile({ userId }: { userId: string  }) {
  const {
    ...
    params: { id: userId },
    onError: (error) => {
      // ✅ Type-safe error handling with feature-specific codes
      switch (error.code) {
        case "USER_NOT_FOUND":
