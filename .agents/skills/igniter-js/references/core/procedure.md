      ...
      procedures: {
        auth: {
          getCurrentSession: async () => {
            ...
            // 4. Verify Token
            const payload = token
              ? await context.fractal.jwt.verifyToken<{ userId: string }>(
                  token,
                )
              : null;
      ...
      procedures: {
        user: {
          createUser = async (data: CreateUserDTO) => {
            ...
            // 2. Hashing Logic
            const hashedPassword = await context.fractal.password.hash(data.password);
          ...
          banUser = async (userId: string, reason: string) => {
              ...
              await context.fractal.mailer.sendBanNotice(user.email, reason);
          };,
    ...
    return {
      procedures: {
        getSessionProject: async ({ userId }: { userId: string }) => {
          ...
          const membership = await context.fractal.projects.getMembership(
            projectId,
            userId,
          );
...
1.  Am I accessing data from `ctx.context.repositories` or `ctx.context.fractal`? (Yes = Good)
