...
- **Never Instantiate in Procedures**: Procedures must NEVER instantiate repositories or services using `new ClassName()`. They should only access pre-instantiated singletons from the global context via `context.services` or `context.repositories`.
      ...
      procedures: {
        auth: {
          getSession: async () => {
            ...
            if (sessionToken) {
              const decoded = context.services.jwt.verifyToken(sessionToken);
          ...
          signIn: async (email: string, password: string) => {
            ...
            const isPasswordValid =
              await context.services.password.comparePassword(
                password,
                user.passwordHash,
              );
            if (!isPasswordValid) {
            ...
            const token = context.services.jwt.generateToken(
              { userId: user.id },
              { expiresIn: "1h" },
            );
          ...
          signUp: async (name: string, email: string, password: string) => {
            ...
            const passwordHash =
              await context.services.password.hashPassword(password);
