...
- [2026-06-23 17:30:00] - **OneDrive reorganization complete (Symlink):** Symlink movido de `.fractal/artifacts/one-drive` → `.fractal/drive`. Todas as 10 referências em toolsets, collections (quotes JSON), skills (SKILL.md) e memories foram atualizadas.
- [2026-06-23 17:35:00] - **Customer slugs aligned with OneDrive:** Slugs `wise`, `or-ryt`, `gtz-84` já correspondem às pastas `WISE`, `OR-RYT`, `GTZ-84` no OneDrive. Templates do Wise copiados da GTZ (mesmo template, mesmo tamanho 8230 bytes).
- [2026-06-23 17:40:00] - **Fixed 10 corrupted quote JSON records:** O sub-agent que atualizou as referências do symlink danificou 4 records (missing commas) e outros 6 records estavam com paths duplicados/concatenados de operações anteriores. Todos corrigidos e validados com `json.load()`.
- [2026-06-23 17:45:00] - **generate_order tool verified:** Já utiliza `.fractal/drive/` como root e segue o padrão de nomenclatura do Drive skill (PEDIDO_{NUMBER}_{TYPE}_{OBRA}_{SUPPLIER}_{REFERENCE}.xlsx).
- [2026-06-24 09:15:00] - **Quote processing v2:** A skill `.fractal/skills/quote-processing` passou a cobrir ingestão de e-mail, reconciliação de collections + `.fractal/drive`, par de evidência `Orcamento-{Fornecedor}-{Referencia}.pdf/.xlsx`, auditoria de pedido e geração do pacote `Mapa de Cotação/data.json + DASHBOARD.html + DASHBOARD.xlsx`. O toolset agora expõe `audit_pedido`, `generate_quote_spreadsheet`, o alias legado `generate_order` e `generate_map_package`.
- [2026-06-24 09:15:00] - **Mapa de cotação como pacote canônico:** antes de gerar, solicitar/confirmar `number`, `type` e `customer`; e-mail é evidência original, collections são o registro estruturado e `.fractal/drive/Obras/{OBRA}/Pedidos/{NUMBER}-{TYPE}` é a hierarquia operacional. O HTML aprovado foi transformado em asset reutilizável com links relativos para `DASHBOARD.xlsx` e `data.json`.
- [2026-06-19 10:30:00] - Identificado template operacional `.fractal/artifacts/docs/customers/or-ryt-paulista/templates/PEDIDO.xlsx`: planilha de pedido de compra com 1 aba (`Baume`), placeholders para automação (`quote.*`, `supplier.*`, `datetime`) e cálculos simples de subtotal/frete/total para uso no fluxo do cliente OR-YT Paulista.
- [2026-06-19 11:10:00] - Gerados pedidos separados para o cliente OR Ryt Paulista a partir do orçamento FF Guarulhos nº 847.627: `PEDIDO-002-HIDRAULICA-OR-RYT-PAULISTA.xlsx` (R$ 38.713,05) e `PEDIDO-003-ELETRICA-OR-RYT-PAULISTA.xlsx` (R$ 3.263,35), com divisão por disciplina e preenchimento do template operacional.
- [2026-06-19 11:35:00] - Corrigido o gerador `PEDIDO.script.py` do cliente OR Ryt Paulista para preservar melhor o layout visual dos pedidos: área-base de itens com espaçamento antes dos totais e reaproveitamento do padrão visual de um pedido de referência já existente do cliente.
- [2026-06-23 10:01:19] - Sincronizada a cotação Fixagold `3029615` do GTZ 84 como pedido `002-Fixação`; registro criado na collection `quotes` e planilha gerada em `.fractal/artifacts/docs/customers/gtz-84/orders/PEDIDO-002-Fixação-gtz-84.xlsx`.
...
- [2026-05-21 08:48:06] - Atlas foi refinado para atuar como assistente pessoal da Dema, com comportamento Karpathy-style: declarar suposições, pedir esclarecimentos quando necessário, reportar o plano antes de executar e manter mudanças cirúrgicas.
- [2026-06-23 10:13:00] - Criado symlink `.agents/skills` apontando para `.fractal/skills` para espelhar as skills do Fractal no ambiente do Codex.
...
- [2026-06-23 11:03:19] - Arquivo consolidado de cotação da obra Wise foi salvo em OneDrive como `Obras/WISE/Mapa de Cotação/MAPA_DE_COTACAO_REF_PEDIDO_006_ELETRICA_WISE_VILA_CLEMENTINO_16062026.xlsx`; padrão definitivo de nome usa `MAPA_DE_COTACAO_REF_PEDIDO_[Number]_[Type]`, seguido da obra e da data `DDMMYYYY`.
- [2026-06-23 11:03:19] - Criado mini dashboard offline em HTML + JSON para o mapa de cotação da Wise em `Obras/WISE/Mapa de Cotação/`; a visualização usa JSON único com customer, suppliers, quotes e items, e foi pensada para compartilhamento por e-mail sem depender de servidor.
- [2026-06-23 11:03:19] - Evolução do dashboard de mapa de cotação: nova leitura prioriza cesta ótima por menor preço, grupos técnicos por família de compra, resumo executivo lateral e lista separada de pendências sem cotação.
- [2026-06-23 11:20:00] - Diretriz visual para dashboards offline de mapa de cotação: evitar moldura externa e “card dentro de card”; preferir layout minimalista de mesa de decisão, ocupando toda a largura, com KPIs em faixa superior, coluna lateral de decisão e tabelas comparativas por grupo.
- [2026-06-23 12:05:00] - Criado `.xlsx` complementar de dashboard para mapa de cotação da Wise a partir do JSON único: `Obras/WISE/Mapa de Cotação/MAPA_DE_COTACAO_REF_PEDIDO_006_ELETRICA_WISE_VILA_CLEMENTINO_16062026_DASHBOARD.xlsx`; padrão proposto: abas `Dashboard`, `Mapa de Cotacao` e `Dados`, preservando a ID visual do HTML com blocos pastel, fornecedor fixo por coluna, destaque verde para menor valor e coluna de menor total.
- [2026-06-24 09:15:00] - O destino vigente dos mapas não é mais a raiz da obra: usar `.fractal/drive/Obras/{OBRA}/Pedidos/{NUMBER}-{TYPE}/Mapa de Cotação/`. Os nomes curtos `DASHBOARD.html`, `DASHBOARD.xlsx` e `data.json` formam a versão compartilhável atual; nomes longos podem coexistir como arquivo histórico.
