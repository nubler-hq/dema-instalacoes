...
export function CasesPageClient() {
  ...
  const filteredCases = useMemo(() => {
    ...
    return allCases.filter((item) => {
      ...
      if (filter === "comercial") {
        return (
          segment.includes("comercial") ||
          segment.includes("mixed-use") ||
          segment.includes("corporativo")
        );
      }
    ...
    <>
      <section>
        <div className="shell section-space pt-24! lg:pt-32!">
          ...
          <h1 className="reveal-up reveal-delay-1 section-title">
            São <strong>+20 anos de experiência</strong> em instalações elétricas
            e hidráulicas.
          </h1>
          <p className="reveal-up reveal-delay-3 mt-6 max-w-2xl text-lg leading-8 text-muted-foreground">
            Uma seleção de projetos que demonstram nosso compromisso com a
            excelência técnica e a satisfação dos nossos clientes.
          </p>
        </div>
      </section>
      ...
      <section>
        <header className="shell pt-0! pb-2!">
          <h2 className="section-title reveal-up reveal-delay-3 text-xl! font-bold">
            Filtros
          </h2>
        </header>
        <div className="shell pt-0!">
          ...
          >
            <ToggleGroupItem
              value="all"
              className="px-6 h-10 text-xs font-semibold uppercase tracking-widest"
            >
              Todos
            </ToggleGroupItem>
            <ToggleGroupItem
            ...
            filteredCases.map((item, index) => (
              ...
              >
                ...
                >
                  <Image
                    src={item.coverImage}
                    alt={`${item.title} — ${item.neighborhood}, São Paulo`}
                    fill
